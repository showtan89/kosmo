...
<body>
	...
	<!-- ##### Breadcrumb Area End ##### -->
		...
		<div class="row" style="margin:20px 0; display:block;" align="right">
			<div class="col-12 col-md-3 col-lg-3" style=" margin:30px 0;float:left;" align="center">
				<div align="center" style="display:block;">
					<input type="text" placeholder="Please enter an address." id="ehddnr" style="height:50px;width:100%;"><br><br>
					<input type="button" class='btn alazea-btn' onclick="initGeocoder();"  style="width:100%;" value="From address"><br><br>
					<input type="text" placeholder="Please enter an address." id="ehddnr2" style="height:50px;width:100%;"><br><br>
					<input type="button" class='btn alazea-btn' onclick="initGeocoder2();" style="width:100%;" value="Arrival address"><br><br>
					<input type="button" class='btn alazea-btn' onclick="searchjido();" style="width:100%;" value="Get Directions">
					<input type="hidden" value="" id="x1">
					<input type="hidden" value="" id="y1">
					<input type="hidden" value="" id="x2">
					<input type="hidden" value="" id="y2">
				</div>
				<div  id="searchjson" align="center" style="margin-top:20px;height:400px;">
				</div>
			</div>
					...
					success : function(obj) {
						...
						for(var i=0;i<path.length;i++){
							...
							if(info.totalDistance>1000){
								...
								str += '<p style="width:100%;margin:0;" align="left"><span style="border:1px solid black;width:25%;color:blue;padding:0 5px;"align="left">Route'+(i+1)+'</span><span style="width:50%;color:red;font-size:20px;margin-left:60px;">&nbsp;&nbsp;About ';
								if(a>0){
									str += a+' Hours '
								}
								str += b+' Minute&nbsp;&nbsp;</span><span style="width:20%;color:black;font-size:18px; float:right" padding-right:20px;>'+info.payment+'원</span></p>';
								 str += '<div style="border:1px solid black;position:absolute;top:-35px;right:-205px;"><a  id="styleblock'+i+'" onclick="Detail(Detailedpath'+i+',stylenone'+i+',styleblock'+i+');">&nbsp;<img style="width:25px;height:25px;" src="resources/img/gido/gidosearch.png"></a></div>';
								str += '<div style="border:1px solid black;position:absolute;top:-35px;right:-205px;"><a id="stylenone'+i+'" onclick="Detailnone(Detailedpath'+i+',styleblock'+i+',stylenone'+i+');" style="display:none">&nbsp;<img style="width:22px;height:25px;" src="resources/img/gido/gidosearch2.png">&nbsp;</a></div>';
								 str += '</li>';
								...
								// subPath for문 길찾기 노선 바뀌는 구간 표시 시작
								 ...
								 for(var k=(subPath.length-1);k>=0;k--){
									 ...
									 if(subPath[k].trafficType==2 || subPath[k].trafficType==1){
										 newnum=oldnum;
										 break;
									 } else {
										 oldnum += oldnum+1;
								 ...
								 for(var j=0;j<subPath.length;j++){
								 	if(subPath[j].trafficType==2){
										...
										str += busType[Type]+' No.'+lane[0].busNo+'('+subPath[j].startName+') ';
									...
									if(pOldNum==0){
										if(pnewNum==3){
											...
											if(subPath[p].distance>1000){
									 		...
									 		var Type = parseInt(lane[0].type);
											str += '<p>'+busType[Type]+' No.'+lane[0].busNo+'('+subPath[p].startName+') After boarding, ';
										...
										} else if(pnewNum==3 && subPath[p].distance!=0){
											pOldNum=pnewNum;
											if((p+1)==subPath.length){
												if(subPath[p].distance>1000){
													var pNum = subPath[p].distance/1000;
